...
automatic() {
    ...
    ui_print "- Downloading the root filesystem using the source ${LXC_MIRROR}..."

    imgctl lxc pull ${LXC_OS} -m ${LXC_MIRROR} -v ${LXC_OS_VERSION} -d "$BASE_DIR"
    if [[ $? != 0 ]]; then
        ui_print "- Download failed. Attempting to download the root filesystem using the fallback source ${LXC_MIRROR_FALLBACK}..."
        imgctl lxc pull ${LXC_OS} -m ${LXC_MIRROR_FALLBACK} -v ${LXC_OS_VERSION} -d "$BASE_DIR"
    fi
